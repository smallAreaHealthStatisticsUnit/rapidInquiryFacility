...
* The data is accessed on the RIF server via a view;
* The view is in the *rif_data* schema. This is mandatory on SQL Server. Use of data on the search path 
  is technically possible but needs testing a 
* Use of Postgres foreign data wrappers directly has not been tested although the code exists. Performance
  limitations in tables with many columns mean that it is advised to use materialized views;
* Materialized views must not be used directly and a view created in the *rif_data* schema to access the
  materialized view;
