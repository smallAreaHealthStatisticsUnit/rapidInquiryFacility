   ...
   var _p = {
      ...
      dropDownInputTextEvents: function(dropdown, available) {
         ...
         available.children().on('click', function() {
            ...
            $(input).val($(this).text()).change().addClass("inputBorderSelection");
            $(this).parent().hide();
         });
      },
      ...
      getCheckBoxLabel: function(name, val_txt, id, checked) {
         var c = (checked) ? "checked" : "",
            p = '<input type="checkbox" name="' + name + '" value="' + val_txt + '" id="' + id + '" class="colsChoice" ' + c + ' />' + '<label for="' + id + '" class="colsChoiceLbl">' + val_txt + '</label>';
         return p;
      },
