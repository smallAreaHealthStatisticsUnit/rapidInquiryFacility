RIF.chart.multipleAreaCharts.d3renderer = ( function ( settings, resultSets, data, update ) {
  ...
  var margin = settings.margin,
    ...
    el = settings.element;
  var svg = d3.select( "#" + el ).append( "svg" ).attr( "width", width - margin.left - margin.right ).attr( "height", ( height - margin.top - margin.bottom ) );
  ...
  function Chart( options ) {
    ...
    this.xScale = d3.scale.linear().range( [ 0, this.width ] ).domain( d3.extent( this.chartData.map( function ( d ) {
      return d[ "x_order" ];
    } ) ) );
    ...
    */
    ...
    this.chartContainer.append( "path" ).data( [ this.chartData ] ).attr( "class", "chart" ).attr( "clip-path", "url(#clip-" + this.id + ")" ).attr( "d", this.area );
    ...
    this.chartContainer.append( "g" ).attr( "class", "y axis" ).attr( "transform", "translate(-4,0)" ).call( this.yAxis );
