RIF.chart.pyramid.d3renderer = (function(opt, data) {
   ...
   var myFormatter = function(d) {
      return (d / 1e6 >= 1) ? (d / 1e6 + "M") : (d / 1e3 >= 1) ? (d / 1e3 + "K") : d;
   }
   var xAxis = d3.svg.axis().scale(x).orient("bottom").tickSize(-height).tickFormat(function(d) {
      return myFormatter(+d);
   });
   // An SVG element with a bottom right origin.
   var svg = d3.select('#' + id).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + 20).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
   ...
   var birthyear = birthyears.selectAll(".birthyear").data(agegroups).enter().append("g").attr("class", "birthyear").attr("transform", function(agegroup) {
      return "translate(0," + y(agegroup.agegroup) + ")";
   });
   birthyear.selectAll("rect").data(function(agegroup) {
   ...
   svg.selectAll(".age").data(agegroups).enter().append("text").attr("class", "age").attr("y", function(agegroup) {
      return y(agegroup.agegroup) - 7;
   }).attr("x", -20).attr("fill", "#8F8F8F").attr("dy", ".9em").text(function(agegroup) {
