RIF.chart.multipleAreaCharts.d3renderer = (function(settings, rSet, max, facade) {
   ...
   d3.select("#" + el).append("a").attr("class", "brushButton").text("zoom").on("click", function() {
      brushIsOn = !brushIsOn;
   ...
   var showBrush = function(isBrush, isOverlay) {
      ...
      svg.selectAll("g.context").attr("style", "display:" + brush);
      svg.selectAll(".overlayHover").attr("style", "display:" + overlay);
   };
   var lineColorUpdate = function(col) {
      svg.selectAll(".lineHover").attr("style", "stroke:" + col);
   };
   ...
   var update = function(xVal, txt, set) {
      xOrders[set] = xVal;
      lines[set].attr("transform", "translate(" + xScales[set](xVal) + "," + 0 + ")");
      texts[set].text(txt);
   };
   return function Chart(options) {
      ...
      var xS = d3.scale.linear().range([0, this.width - 40]) //24px is the size of Y axis 
         .domain([1, length]);
      xScales[localName] = xS;
      var yS = d3.scale.linear().range([this.height, 0]).domain([0, this.maxDataPoint]);
      ...
      var brush = d3.svg.brush().x(xS).on("brushend", brushed);
      ...
      var context = this.chartContainer.append("g").attr("class", "context context" + localName).attr("transform", "translate(" + 0 + "," + 0 + ")");
      context.append("g").attr("class", "x axis").attr("transform", "translate(0," + chartHeight + ")").call(xS);
      context.append("g").attr("class", "x brush").call(brush).selectAll("rect").attr("height", chartHeight);
      var chartClass = "areaChart" + ++areaChartsCount;
      this.chartContainer.append("path").data([options.data]).attr("class", chartClass).attr("clip-path", "url(#clip-" + this.id + ")").attr("d", this.area);
      ...
      var rectInteractive = this.chartContainer.append("rect").attr("class", "overlayHover").attr("width", width + 24).attr("height", chartHeight).attr("transform", "translate(0,0)").on("mousemove", mousemove).on("click", mouseclick).on("keydown", keyDown);
      /* Highlighter */
      lines[localName] = this.chartContainer.append("line").attr("class", "lineHover").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", chartHeight).attr("height", 2).attr("height", chartHeight).attr("id", linename);
      this.yAxis = d3.svg.axis().scale(yS).orient("left").tickValues([0, 1, this.maxDataPoint]);
      this.yContainer = this.chartContainer.append("g").attr("class", "y axis").call(this.yAxis);
      this.chartContainer.append("rect").attr("width", 25).attr("class", "yClickWrapper").attr("height", chartHeight).attr("transform", "translate(-25,0)").on("click", mouseclickOnYAxis)
      this.chartContainer.append("text").attr("class", "country-title").attr("transform", "translate(10,20)").text(this.name);
      texts[localName] = this.chartContainer.append("text").attr("class", "areaValue").attr("id", localName + "_text").attr("transform", "translate(10,32)").text("0.00");
   };
});