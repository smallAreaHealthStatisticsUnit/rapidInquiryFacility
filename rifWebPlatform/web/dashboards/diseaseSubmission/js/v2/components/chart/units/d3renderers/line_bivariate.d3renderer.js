RIF.chart.line_bivariate.d3renderer = (function(opt, data, gidSelected) {
   ...
   var x = d3.scale.linear().range([0, xWidth]);
   var x2 = d3.scale.linear().range([0, xWidth]);
   ...
   var svg = d3.select("#" + id).insert("svg", "div").attr("width", width).attr("height", height).attr("id", "lineBivariatesvg");
   svg.append("defs").append("clipPath").attr("id", "clipchart").append("rect").attr("width", width).attr("height", height);
   ...
   focus.append("path").datum(data).attr("class", "area").attr("d", area).attr("clip-path", "url(#clipchart)");
   focus.append("g").attr("class", "x axis").attr("transform", "translate(0," + height + ")").call(xAxis);
   focus.append("g").attr("class", "y axis").call(yAxis)
   svg.append("text").attr("transform", "translate(36,20)").attr("id", "labelLineBivariate").text(lineField);
   var currentFigures = svg.append("text").attr("transform", "translate(36,40)").attr("id", "currentFiguresLineBivariate").text("");
   focus.append("path").datum(data).attr("class", "line").attr("id", "lineRisk").attr("clip-path", "url(#clipchart)").attr("d", line);
   /* Highlighter */
   var highlighter = focus.append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", height).attr("height", height).attr("class", "bivariateHiglighter");
   var pointerLighter = focus.append("circle").attr("r", 3).attr("class", "bivariateHiglighter");
   ...
   var updateLine = function() {
      ...
      currentFigures.text((lookUp[gid][1]).toFixed(3) + " ( " + lookUp[gid][2] + " - " + lookUp[gid][3] + " )");
   };
