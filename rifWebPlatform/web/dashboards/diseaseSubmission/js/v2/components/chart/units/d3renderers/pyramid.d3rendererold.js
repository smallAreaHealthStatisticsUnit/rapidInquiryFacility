RIF.chart.pyramid.d3renderer = (function(opt, data) {
   ...
   var id = opt.element,
      ...
      margin = opt.margin,
      field = opt.field;
   var y = d3.scale.linear().range([barHeight / 2, height - barHeight / 2]);
   ...
   var title = svg.append("text").attr("class", "title").attr("dy", ".71em").attr("x", width).text(2000);
   ...
   svg.append("g").attr("class", "y axis").attr("transform", "translate(0," + height + ")").call(xAxis).selectAll("g").filter(function(value) {
      return !value;
   }).classed("zero", true);
   /* 
    ...
    */
   var birthyear = birthyears.selectAll(".birthyear").data(minage).enter().append("g").attr("class", "birthyear").attr("transform", function(birthyear) {
      return "translate(0," + y(birthyear) + ")";
   });
   birthyear.selectAll("rect").data(function(data) {
      console.log(data); /*return data[year][birthyear] || [0, 0]; */
      return 0;
   }).enter().append("rect").attr("y", -barHeight / 2).attr("height", barHeight).attr("x", 0).attr("width", function(value) {
      return x(value);
   });
   ...
   svg.selectAll(".age").data(d3.range(0, 18, 1)).enter().append("text").attr("class", "age").attr("y", function(minage) {
      return y(minage) - 7;
   }).attr("x", -12).attr("dy", ".9em").text(function(minage) {
      return minage + '-' + minmaxage[minage];
   });
   update();
   ...
   function update() {
      ...
      title.text(year);
      birthyears.transition().duration(750).attr("transform", "translate(0," + (y(year1) - y(year)) + ")");
      birthyear.selectAll("rect").data(function(birthyear) {
         return data[year][birthyear] || [0, 0];
      }).transition().duration(750).attr("x", x(0)).attr("width", function(value) {
         return x(value);
      });
   }
   //});*/
