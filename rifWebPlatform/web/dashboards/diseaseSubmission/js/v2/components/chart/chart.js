RIF.chart = ( function ( settings ) {
  var charts = settings.charts,
    ...
    _p = {
      ...
      updateChart: function ( type, args ) {
        ( type === 'line_bivariate' ) ? _p.updateLine_bivariate( args ) : ( type === 'line_bivariateWithHighlight' ) ? _p.updateLine_bivariateWithClick( args ) : ( type === 'pyramid' ) ? _p.updatePyramid( args ) : ( type === 'histogram' ) ? _p.updateHisto( args ) : ( type === 'multipleAreaCharts' ) ? _p.updateMultipleAreaCharts( args ) : ( type === 'multipleAreaChartsFromMap' ) ? _p.updateAllCharts( args ) : 0;
      },
      clearChart: function ( type, args ) {
        ( type === 'line_bivariate' ) ? _p.clearLineBivariate( args ) : ( type === 'pyramid' ) ? 0 : ( type === 'histogram' ) ? 0 : ( type === 'multipleAreaCharts' ) ? _p.clearMultipleAreaCharts( args ) : 0;
      },
  ...
  _p.init().getFacade().extendChart().setEvents();
  return _p.facade;
} );