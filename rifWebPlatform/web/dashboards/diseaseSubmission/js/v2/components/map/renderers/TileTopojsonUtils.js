RIF.TileTopojsonUtils = (function (tooltip, geolvl, counter, wrapperId) {

  var that = this;

  var slctd = RIF[wrapperId] = []; //RIF[wrapperId] is a RIF global variable which allow the event file to propagfate map area selection to the mediator
  ...
  var _slctFromClick = function (dProperties, isSlctd, id) {
    var id = (typeof id == 'undefined') ?
      tiled.getPathId({
        id: dProperties.gid
      }) : id;
  ...
  var tiled = {
    ...
    slct: function (d) {
      var actualPathId = tiled.getPathId({
        id: d.gid
      });
    ...
    evntHndl: function (c, d) {
      ...
      switch (c) {
      case "click":
        _slctFromClick(d /*properties*/ , isSlctd);
