L.TileLayer.d3_geoJSON = L.TileLayer.extend({
   ...
   _d3Callback: function(tile, tilePoint) {
      ...
      return function(r) {
         ...
         tile.nodes.selectAll("path").data(geoJS).enter().append("path").attr("d", self._path).attr("style", opt.style).attr("class", opt.class).attr("id", opt.id).on("mouseout", function() {
            opt.evntHndl.call(this, "mouseout");
         }).on("mousedown", function() {
            self.drag = -1;
         }).on("mouseup", function() {}).on("mouseover", function() {
