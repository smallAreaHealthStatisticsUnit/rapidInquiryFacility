...
RIF.style.scales = ( function ( params ) {
  var type = params[ "classification" ],
    ...
    scales = {
      quantize: function () {
        var q = d3.scale.quantize().domain( [ min_value, max_value ] ).range( d3.range( intervals ) );
        return q;
      },
      quantile: function () {
        var q = d3.scale.quantile().domain( [ min_value, max_value ] ).range( d3.range( intervals ) );
        return q;
      },
      jenks: function () {
        ...
        var l = jenks.length;
        var j = d3.scale.threshold().domain( jenks ).range( d3.range( intervals + 2 ).map( function ( i ) {
          return i - 1
        } ) );
        return j;
      },
      threshold: function () {
        var t = d3.scale.threshold().domain( domain ) //[.02, .04, .06, .08, .10]
      ...
      standardDeviation: function () {
        ...
        var j = d3.scale.threshold().domain( breaks ).range( d3.range( intervals + 2 ).map( function ( i ) {
          return i - 1
        } ) );
        return j;
      },
