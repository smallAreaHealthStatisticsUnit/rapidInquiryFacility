L.TileLayer.canvasTopojson = L.TileLayer.Canvas.extend({
    ...
    _isActuallyVisible: function (coords) {
        var coord = coords[0],
            min = [coord.x, coord.y], max = [coord.x, coord.y];
    ...
    _draw: function (ctx, tilePoint, zoom) {
        ...
        var loader = d3.xhr;
        loader.error = function(  jqXHR,  textStatus,  errorThrown ){console.log(textStatus + " " + errorThrown)};
        ...
        loader(url, function (data) {
		    ...
		    var geoJS = topojson.feature( data, o );
			/*geoJS.filter(function(d) {
			   if (d.id <1000 ) {return true}; 
			   return false;
			});*/
			//path( geoJS, 0 )
