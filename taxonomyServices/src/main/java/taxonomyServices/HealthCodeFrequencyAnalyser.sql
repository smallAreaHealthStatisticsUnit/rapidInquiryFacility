 ...
 * and year ranges, for any given geographical area.  This particular utility
 * concerns the management of taxonomy services that provide those health
 * outcomes to users.
 *
...
CREATE OR REPLACE FUNCTION count_non_empty_diagnostic_codes(
	diag_01 TEXT,
	diag_02 TEXT,
	diag_03 TEXT)
	RETURNS INT AS 
...
BEGIN
	...
	SELECT
		year,
		healthcare_provider,
		count_non_empty_diagnostic_codes(
			diag_01, 
			diag_02, 
			diag_03) AS non_empty_diagnostic_codes
	FROM
		patient_records;
	...
	SELECT
		...
		year,
		non_empty_diagnostic_codes,
		COUNT(non_empty_diagnostic_codes) AS frequency			
	FROM
	...
	ORDER BY
		healthcare_provider ASC,
		year DESC,
		non_empty_diagnostic_codes DESC;
	...
	SELECT
		...
		count_non_empty_diagnostic_codes(
			oper_01, 
			oper_02) AS non_empty_operation_codes
	FROM
		patient_records;
	...
	SELECT
		...
		year,
		non_empty_operation_codes,
		COUNT(non_empty_operation_codes) AS frequency			
	FROM
	...
	ORDER BY
		healthcare_provider ASC,
		year DESC,
		non_empty_operation_codes DESC;
...
-- #RUN_PROGRAM:
-- Uncomment one of these lines.  Then either run the script from command
-- line or copy all of it into pgAdmin's Query Tool window.  If you run
-- it with a CSV file, make sure it has the columns:
