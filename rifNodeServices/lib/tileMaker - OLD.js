...
var dbTileMaker = function dbTileMaker(dbSql, client, createPngfile, tileMakerConfig, dbType, dbTileMakerCallback, maxZoomlevel, blocks, winston) {
	...
	var dbErrorHandler = function dbErrorHandler(err, sqlInError, callback) {
		var nerr;
		if (err) {	
			nerr=new Error("dbErrorHandler(): " + err.message + "\nStack:\n" + err.stack + "\nSQL> " + (sqlInError||sql));
...
//			winston.log("error", "dbErrorHandler() Error: " + nerr.message);
	...
	function Tile(row, geojson, topojson_options, tileArray) { 
		...
		this.next_block=row.next_block;
...
//		}
		...
		if (tileArray.length == 0) { // First row in zoomlevel/geolevel combination
			...
			var newTile="false (" + row.block + ")";
			if (row.next_block != row.block) {
				newTile="true(" + row.next_block + "=>" + row.block + ")";
			}
			if (geojsonTile.tileId == row.tile_id) { 	// Same tile
				if (geojson) {
					geojsonTile.addFeature(row, geojson);	// Add geoJSON feature to collection
					winston.log("debug", 'Tile ' + geojsonTile.id + '(newTile: ' + newTile + '); add areaID: ' + row.areaid + ": " + 
				...
				for (i=0; i<tileArray.length; i++) {	// Tiles should be in order, but in case the DB gets it wrong (Postgres did!)
					if (tileArray[i].tileId == row.tile_id) { 	// Same tile
						...
						geojsonTile.addFeature(row, geojson);	// Add geoJSON feature to collection
						foundTile=true;
						winston.log("debug", 'Old Tile ' + geojsonTile.id + '(newTile: ' + newTile + '); add areaID: ' + row.areaid + ": " + 
							geojsonTile.geojson.features[(geojsonTile.geojson.features.length-1)].properties["areaName"]);
					}
				}
				if (foundTile) {
					winston.log("debug", 'NewTile: ' + newTile);
...
//			
		if (dbType == "PostGres") {	
			sql="WITH a AS (\n" +
				...
				"	       z.areaid, z.geolevel_id, z.zoomlevel, z.optimised_wkt, z.x, z.y, y.block, y.next_block, a.*\n" +				
				...
				")\n" +
				"SELECT * FROM a\n" + 
				" ORDER BY block, tile_id, areaid";
		}		
		else if (dbType == "MSSQLServer") {		
			sql="WITH a AS (\n" +
				"	SELECT CAST(z.geolevel_id AS VARCHAR) + '_' + '" + (geolevelName||'Unknown geolevel name') + 
								"' + '_' + CAST(z.zoomlevel AS VARCHAR) + '_' + CAST(z.x AS VARCHAR) + '_' + CAST(z.y AS VARCHAR) AS tile_id,\n" +
				"	       z.geolevel_id, z.zoomlevel, z.optimised_wkt, z.areaid, z.x, z.y, y.block, y.next_block, a.*\n" +				
				"	  FROM " + tileIntersectsTable + " z, " + tileBlocksTable + " y, lookup_" + geolevelName + " a\n" +
			...
			if (insertArray.length == 0) {
				...
				var query=request.query(pgInsertSql, insertArray, function pgTilesInsert(err, result) {
					if (err) {
						...
						dbErrorHandler(new Error("pgTilesInsert() tile INSERT: result undefined != expected: " + expectedRows), pgInsertSql);
						
					}
					else if (expectedRows != result.rowCount) {
						dbErrorHandler(new Error("pgTilesInsert() tile INSERT: " + result.rowCount + " != expected: " + expectedRows), 
							pgInsertSql);
					}
					else {					
						var removed=0;
						for (var i=0; i<tileArray.length; i++) {
							if (tileArray[i].topojson) { 
								tileArray.splice(i);	// Re-initialize tile array - remove processed tiles
								removed++;
							}
						}		
						winston.log("verbose", "pgTileInsert() " + removed + " Inserts OK; " + tileArray.length + " to process");	
	...
	//										"): " + data.optimised_topojson.substring(0, 2500));
				...
				function mssqlTileInsertEnd(err) { //  Callback	
					if (err) {
						...
						dbErrorHandler(new Error("mssqlTilesInsert() tile INSERT: " + j + " != expected: " + expectedRows), insertSql, mssqlTileInsertCallback);
					}
						...
						winston.log("verbose", "mssqlTileInsertEnd() [" + removed + "]: Inserts OK; " + tileArray.length + " to process");	
						...
						async.forEachOfSeries(HY104Sql, 
							...
							function mssqlTileInsert2End(err) { //  Callback	
								if (err) {
									...
									winston.log("verbose", "mssqlTileInsert2End() [" + tileArray.length + "]: Inserts OK; " + HY104Sql.length + " HY104 redo inserts");
		...
		function tileIntersectsProcessingEnd(rowCount, dbType, tilesCsvStream, block, numBlocks, getBlockCallback) {
			...
			if (tileArray.length == 0) {
				...
				if (geojsonTile) {
					if (geojsonTile.block != geojsonTile.next_block) {
						winston.log("debug", "Process final tile");
						totalTileSize+=geojsonTile.addTopoJson();				// Complete final tile	
					}
					else {
						winston.log("debug", "Unable to process final tile: block (" + 
							geojsonTile.block + ") != next_block (" + geojsonTile.next_block + ")");
					}
				}	
				...
				winston.log("info", 'Geolevel: ' + geolevel_id + '; zooomlevel: ' + zoomlevel + '; ' + 
					'block: ' + block + '/' + numBlocks + '; ' + 
					rowCount + ' tile intersects processed; ' + processable + "/" + tileArray.length + " tiles in " + elapsedTime + " S; " +  
					Math.round((tileArray.length/elapsedTime)*100)/100 + " tiles/S" + 
					"; total: " + tileNo + " tiles in " + tElapsedTime + " S; size: " + (nodeGeoSpatialServicesCommon.fileSize(totalTileSize)||"0 bytes"));
				...
				function csvFileCallback() {
					if (createPngfile) { // Create SVG tiles	
						...
						function writeSVGTileSeries(value, j, writeSVGTileCallback) { // Processing code	
							// Write completed SVG tiles to disk
								if (value.topojson) {
				...
				if (boundData.length < 1) {
					if (geolevel_id == 1 && zoomlevel > 0) {
						...
						dbErrorHandler(new Error("getBlock() table: tile_blocks_" + geography + 
							" fetch rows <1 (" + boundData.length + ") for geolevel_id: " + geolevel_id + "; zoomlevel: " + zoomlevel), blockSql);
					}	
				}		
				else {				
