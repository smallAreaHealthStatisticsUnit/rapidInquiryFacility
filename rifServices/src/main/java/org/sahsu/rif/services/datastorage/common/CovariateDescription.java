...
class CovariateDescription {
	...
	private SelectQueryFormatter covariateTableFormatter;
	...
	CovariateDescription(final SQLManager manager, final Connection connection,
		...
		covariateTableFormatter = SelectQueryFormatter.getInstance(dbType);
	...
	String get() throws SQLException {
		...
		PreparedStatement ps = SQLQueryUtility.createPreparedStatement(connection,
		                                                               covariateTableFormatter);
