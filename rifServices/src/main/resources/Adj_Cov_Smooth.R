...
#library(Matrix)
library(RODBC)
#will need db libraries as well

# change as appropriate
setwd('C:/GitHub/rapidInquiryFacility/rifStats/rif4')

#make and ODBC connection
connDB = odbcConnect("networkRif")
#data=sqlFetch(connDB, "rifstudy")
data=sqlFetch(connDB, "rif_studies.s1_extract")

# use this code to get the adjacency matrix: sqlQuery(connDB, paste("SELECT * FROM rif40_xml_pkg.rif40_GetAdjacencyMatrix(1) LIMIT 10"))

#for now, read in csv file of data - replace with odbc calls
...
# write.table(RES,file='Export_RresultsADJ.csv', sep=',', row.names=FALSE)
#########################################################################################
# now start the smoothing bit
data = RES

#########################################################################################
#SELECT MODEL - this should be passed in to the program somewhow.
model='BYM' #Either 'BYM', 'CAR' or 'HET'
#SELECT adj or not adj should be a parameter
...
AdjRowset<-sqlQuery(connDB, paste("SELECT * FROM rif40_xml_pkg.rif40_GetAdjacencyMatrix(1)"))
...
# write the results to a file for now - should go to a DB
write.table(data,'export_Adj_Cov_Smooth.csv',sep=',',row.names=FALSE)

# write the results to a new database table
#sqlDrop(connDB, "rifSmoothTest", errors = FALSE)
#sqlSave(connDB, data, tablename = "rifSmoothTest")
