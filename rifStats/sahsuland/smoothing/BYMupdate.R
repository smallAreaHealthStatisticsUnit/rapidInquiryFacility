...
Check_CVG=function(SET){
  n=length(SET)
  mcl=list()
  for (c in 1:n){
    mcl[[c]]=mcmc(cbind(SET[[c]]$ALPHA,SET[[c]]$SIG2,SET[[c]]$TAU2,t(SET[[c]]$U),t(SET[[c]]$V)))
  }
  mcObj=mcmc.list(mcl)
  gelman.diag(mcObj,multivariate=FALSE)
}
#Function for checking convergence
Extract_RES=function(SET){
  n=length(SET)
  nit=length(SET[[1]]$ALPHA)
  mcl=list()
  for (c in 1:n){
    mcl[[c]]=mcmc(cbind(SET[[c]]$ALPHA,SET[[c]]$SIG2,SET[[c]]$TAU2,t(SET[[c]]$U),t(SET[[c]]$V)),start=floor(nit/2),end=nit)
  }
  mcObj=mcmc.list(mcl)
  return(mcObj)
}
