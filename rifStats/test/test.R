...
SETlist=list()
for (c in 1:3){
  SETlist[[c]]=initialize(log(data$y/data$E+0.00001))
}
M=prec_Matrix(LDN.adj)
cl <- makeCluster(3)
...
if (InferenceMethod=='MCMC_MH'){
  SET=parLapply(cl,SETlist,fun=BYM.UPDATE_MH,Y=data$y,E=data$E,M=M,Nsimu=10000
                ,au=0.001,bu=0.001,av=0.001,bv=0.001)
}
...
BYM_ssRR=colMeans(ssRR)
maps.SMR <- data.frame(NAME=data$NAME,ssRR=BYM_ssRR)
attr(boroughs, "data")=merge(data.boroughs,maps.SMR.factors,by="NAME")
trellis.par.set(axis.line=list(col=NA))
spplot(obj=boroughs, zcol= "SMR.DM", col.regions=gray(3.5:0.5/4),main="")
