...
WITH n1 AS (    /* NUM_SAHSULAND_CANCER - cancer numerator */
        SELECT s.area_id                /* Study or comparision resolution */,
               ...
               SUM(CASE                 /* Numerators - can overlap */
                        ...
                        AND (1=1
                           ...
                           AND  FLOOR(c.age_sex_group/100) = 1
         ...
         WHERE c.sahsu_grd_level1 = s.area_id   /* Comparison selection */
           ...
           AND c.year BETWEEN @yearstart AND @yearstop  /* Denominator (INSERT) year filter */
        ...
        SELECT d1.year, s.area_id, CAST(NULL AS INTEGER) AS band_id, d1.age_sex_group,
          ...
          FROM ##g_rif40_comparison_areas s, rif_data.pop_sahsuland_pop d1      /* Study or comparison area to be extracted */
...
WITH n1 AS (    /* NUM_SAHSULAND_CANCER - cancer numerator */
        SELECT s.area_id                /* Study or comparision resolution */,
               ...
               SUM(CASE                 /* Numerators - can overlap */
                        WHEN (( /* Investigation 1 ICD filters */
                                    icd LIKE 'C34%' /* Value filter */ /* Filter 1 */) /* 1 lines of conditions: study: 7, inv: 7 */
                        AND (1=1
                                        /* No year filter required for investigation 1 */
                           AND  TRUNC(c.age_sex_group/100) = 1
                                        /* No age group filter required for investigation 1 */)
                        ) THEN total
                        ELSE 0
               END) inv_7_test_1001     /* Investigation 1 -  */
...
SELECT d.year,
       ...
       COALESCE(n1.inv_7_test_1001, 0) AS inv_7_test_1001,
       d.total_pop
  FROM d                        /* Denominator - population health file */
...
| Summary Counts      | C/Males  | C/Females| S/Males  | S/Females|
|---------------------|----------|----------|----------|----------|
...
| max_year            | 1995     | 1995     | 1995     | 1995     |
| min_age_group       | 0        | 0        | 0        | 0        |
| max_age_group       | 21       | 21       | 21       | 21       |
| test_1001           | 4324     | 0        | 4324     | 0        |
| total_pop           | 35336652 | 37152808 | 35336652 | 37152808 |
...
**test_1001 C/males disagrees with manual calulation: 63216**

Old - not using total field
