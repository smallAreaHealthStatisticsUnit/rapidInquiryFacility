...
declare @investigation_state_only_flag int -- MIGHT NOT WORK FOR BATCH INSERTS 
	IF exists (
			select ic.username  
		    from inserted ic , [rif40].[t_rif40_investigations] iv
...
WHERE	table_name in (select [NUMER_TAB] from [rif40].[t_rif40_investigations] where ISNUMERATOR <>1)
 FOR XML PATH('') 
);
BEGIN TRY
...
FROM	[rif40].[rif40_tables] a 
inner join [rif40].[t_rif40_investigations] b
