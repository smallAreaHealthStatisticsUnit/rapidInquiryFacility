...
-- psql -U rif40 -d sahsuland -w -e -P pager=off -f alter_scripts/v4_0_alter_14.sql
--
...
COMMENT ON COLUMN rif40_studies.stratify_to IS 'Stratification to perform; risk analysis only.
The stratificationType is one of:
* "NONE": No stratification;
* "GEOLEVEL": Stratification by geolevel, usually region. Geolevels 2 and 3 are available for choice;
* "MULTIPOLYGON": Stratification by a single multi polygon site;
...
COMMENT ON COLUMN rif40_studies.stratification_list IS 'List of stratifications available. E.g.
	[{
		"name": "NONE",
		"stratificationType": "NONE",
		"description": "No stratification"
...
COMMENT ON COLUMN rif40_studies.stratification_field 
	IS 'Stratification field if selected from shapefile; risk analysis only';
...
--
		IF (USER = NEW.username OR NEW.username IS NULL /* Will be defaulted */) AND rif40_sql_pkg.is_rif40_user_manager_or_schema() THEN
			...
			VALUES(
				...
				NEW.stratification_field,
				NEW.stratify_to,
				NEW.stratification_list);
		ELSE
		...
		IF USER = OLD.username AND NEW.username = OLD.username THEN
			UPDATE t_rif40_studies
			   SET username=NEW.username,
				   ...
				   stratification_field=NEW.stratification_field,
				   stratify_to=NEW.stratify_to,
				   stratification_list=NEW.stratification_list
...
--
		IF (USER = NEW.username OR NEW.username IS NULL /* Will be defaulted */) AND rif40_sql_pkg.is_rif40_user_manager_or_schema() THEN
			...
			VALUES(
				...
				NEW.stratification /* no default value */);
		ELSE
