...
CREATE OR REPLACE FUNCTION rif40_sql_pkg._rif40_hash_partition_create_insert(partition_schema VARCHAR, master_schema VARCHAR,
...
Parameters:	Partition schema, master table schema, table, column, total rows, number of partitions
...
--
				IF c3_rec.column_names IS NOT NULL THEN
					ddl_stmt[array_length(ddl_stmt, 1)+1]:='INSERT INTO '||quote_ident(partition_schema)||'.'||quote_ident(l_table)||'_p'||c6_rec.partition_value||
				...
				END IF;
				ddl_stmt[array_length(ddl_stmt, 1)+1]:='UPDATE '||quote_ident(partition_schema)||'.'||quote_ident(l_table)||'_p'||c6_rec.partition_value||E'\n'||
...
--
	l_ddl_stmt:=rif40_sql_pkg._rif40_hash_partition_functional_index(partition_schema, l_table, l_column, num_partitions);
	IF l_ddl_stmt IS NOT NULL THEN
--
-- Copy out parameters
--
...
COMMENT ON FUNCTION rif40_sql_pkg._rif40_hash_partition_create_insert(VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER,
