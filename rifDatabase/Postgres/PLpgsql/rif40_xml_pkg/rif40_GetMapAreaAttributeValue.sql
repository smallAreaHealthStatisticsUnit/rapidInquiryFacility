...
a) This function can be slow as if uses rif40_num_denom, takes 398mS on my laptop to fetch rows to RI40_NUM_DEMON based (health and population) themes
...
psql:alter_scripts/v4_0_alter_1.sql:542: INFO:  [DEBUG1] rif40_GetMapAreaAttributeValue(): [50212] Neither gid or gid_rowindex columns are present in attrribute source table: sahsu
land_pop
...
psql:alter_scripts/v4_0_alter_1.sql:542: INFO:  [DEBUG1] rif40_GetMapAreaAttributeValue(): [51221] Cursor: c4getallatt4theme_3, geography: SAHSU, geolevel select: LEVEL2, theme: po
...
psql:alter_scripts/v4_0_alter_1.sql:506: INFO:  [DEBUG1] rif40_GetMapAreaAttributeValue(): [50212] Neither gid or gid_rowindex columns are present in attrribute source table: sahsu
land_covariates_level4
psql:alter_scripts/v4_0_alter_1.sql:506: INFO:  [DEBUG1] rif40_GetMapAreaAttributeValue(): [51219] c4getallatt4theme SQL>
WITH a AS (
...
psql:alter_scripts/v4_0_alter_1.sql:506: INFO:  [DEBUG1] rif40_GetMapAreaAttributeValue(): [51221] Cursor: c4getallatt4theme_1, geography: SAHSU, geolevel select: LEVEL4, theme: co
...
--
		PERFORM rif40_log_pkg.rif40_error(-51214, 'rif40_GetMapAreaAttributeValue', 
			'Invalid theme: %',
			l_theme::VARCHAR);
	END IF;
--
...
--
	IF rif40_log_pkg.rif40_is_debug_enabled('rif40_GetMapAreaAttributeValue', 'DEBUG2') THEN
		BEGIN
--
-- Create temporary table
--
			PERFORM rif40_log_pkg.rif40_log('DEBUG1', 'rif40_GetMapAreaAttributeValue', '[51215] c4getallatt4theme EXPLAIN SQL> '||E'\n'||'%;', sql_stmt::VARCHAR); 
			sql_stmt:='EXPLAIN ANALYZE VERBOSE CREATE TEMPORARY TABLE '||LOWER(quote_ident(c4getallatt4theme::Text))||E'\n'||'AS'||E'\n'||sql_stmt;
--
-- Create results temporary table, extract explain plan  using _rif40_GetMapAreaAttributeValue_explain_ddl() helper function.
-- This ensures the EXPLAIN PLAN output is a field called explain_line 
--
			sql_stmt:='SELECT explain_line FROM rif40_xml_pkg._rif40_GetMapAreaAttributeValue_explain_ddl('||quote_literal(sql_stmt)||', $1, $2, $3, $4)';
...
--
				RAISE;
		END;
		PERFORM rif40_log_pkg.rif40_log('DEBUG2', 'rif40_GetMapAreaAttributeValue', 
...
--
		BEGIN	
			...
			OPEN c4getallatt4theme SCROLL FOR EXECUTE sql_stmt /* Make cursor scrollable */;
		EXCEPTION
			WHEN others THEN
--
-- Print exception to INFO, re-raise
...
--
			...
			OPEN c4getallatt4theme SCROLL FOR EXECUTE sql_stmt /* Make cursor scrollable */ USING l_geography, l_geolevel_select, l_offset, l_row_limit;
		EXCEPTION
			WHEN others THEN
--
-- Print exception to INFO, re-raise
...
CREATE OR REPLACE FUNCTION rif40_xml_pkg._rif40_GetMapAreaAttributeValue_explain_ddl(
...
COMMENT ON FUNCTION rif40_xml_pkg._rif40_GetMapAreaAttributeValue_explain_ddl(VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER) IS 'Function: 	rif40_GetMapAreaAttributeValue_explain_ddl()
Parameters:	SQL statement, geography, geolevel select, offset, row limit
Returns: 	TABLE of explain_line
Description:	Coerce EXPLAIN output into a table with a known column. 
		Supports EXPLAIN and EXPLAIN ANALYZE for rif40_geolevelAttributeTheme() ONLY.';
