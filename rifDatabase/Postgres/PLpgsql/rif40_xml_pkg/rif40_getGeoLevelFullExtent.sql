...
CREATE OR REPLACE FUNCTION rif40_xml_pkg.rif40_getGeoLevelFullExtent(
	l_geography 	VARCHAR,
...
--
	IF NOT rif40_sql_pkg.is_rif40_user_manager_or_schema() THEN
		PERFORM rif40_log_pkg.rif40_error(-50000, 'rif40_getGeoLevelFullExtent', 
	...
	c1_rec:=rif40_xml_pkg._rif40_getGeoLevelExtentCommon(l_geography, l_geolevel_view, 
		NULL /* map area (single <geolevel view> area id) */, NULL /* or, study id */);
--
...
COMMENT ON FUNCTION rif40_xml_pkg.rif40_getGeoLevelFullExtent(VARCHAR, VARCHAR, 
	OUT REAL, OUT REAL, OUT REAL, OUT REAL) IS 'Function: 	rif40_getGeoLevelFullExtent()
Parameters:	Geography, geolevel_view
Returns:	Y max, X max, Y min, X min as a record. Cast to REAL (6 decimal digits precision)
Description:	Get bounding box Y max, X max, Y min, X min for <geography> <geolevel view>
		SRID is 4326 (WGS84)
...
GRANT EXECUTE ON FUNCTION rif40_xml_pkg.rif40_getGeoLevelFullExtent(VARCHAR, VARCHAR, 
