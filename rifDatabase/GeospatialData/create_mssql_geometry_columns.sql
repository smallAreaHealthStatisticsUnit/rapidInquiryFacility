...
-- MS SQL Server specific parameters
--
-- Usage: sqlcmd -U rif40 -d <database name> -b -m-1 -e -r1 -i create_mssql_geometry_columns.sql
...
--
	SET @run_update = 'UPDATE geometry_columns' + @crlf +
	'   SET coord_dimension = COALESCE(' + CAST(@Selected_geometry_dimension AS VARCHAR) + ', 2),' + @crlf +
    '       srid 		   = ' + CAST(@Selected_geometry_srid AS VARCHAR) + ',' + @crlf +
	'	   geometry_type   = ''' + @Selected_geometry_type + '''' + @crlf +
	'	WHERE CURRENT OF c1'; 
	PRINT 'SQL> ' + @run_update;
